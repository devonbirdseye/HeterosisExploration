...
PtEncodeRibo<- PtGenes$Gene[(grep("description:rps|description:rpl|description:ribosom", PtGenes$Protein))]
```
...
ProkRibo <- df.acs.SL[(grep("30S|60S", df.acs.SL$Protein)),]
...
ProkRiboNotPt[(ProkRiboNotPt$Gene%in%GOplastid),]
```

```{r}
#Eukaryotic ribosomal proteins (EnsemblPlants Annotation)
eukRibo<-df.acs.SL$Gene[(grep("40S|60S", df.acs.SL$Protein))]
```
...
fig4 <- ggplot(df.acs.SL.lims, mapping=aes(x=l2fc.acs, y=l2fc.BM, color=Category))+
  geom_point(size=.8)+
  scale_color_manual(values=c("Other"="grey80", "PlastidEncodedPS"="red4", "PHANGs"="lightcoral", "PlastidEncodedRibo"="blue4", "NuclearEncodedPlastidRibo"="dodgerblue1", "CytosolicRibo"="green4"), labels=c("Other"="Other", "PlastidEncodedPS"="Plastid-encoded photosynthesis-associated", "PHANGs"="Nuclear-encoded photosynthesis-associated", "PlastidEncodedRibo"="Plastid-encoded ribosome", "NuclearEncodedPlastidRibo"="Nuclear-encoded plastid ribosome", "CytosolicRibo"="Cytosolic ribosome"), name="Protein Category")+
  theme_bw()+
  xlab("log2(acs/B73)")+
  ylab("log2(BxM/MP)")+

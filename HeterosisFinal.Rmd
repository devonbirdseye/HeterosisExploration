...
3. Plastid encoded ribosomal proteins + 3"other"(= matK, infA, and clpP, added based on expression grouping) (EnsemblPlants Annotation)
4. Plastid encoded RNA polymerases (EnsemblPlants Annotation)
5. Nuclear encoded plastid ribosomal proteins (GO GAMER and EnsemblPlants Annotation)
6. Eukaryotic ribosomal proteins (EnsemblPlants Annotation)
...
fig4 <- ggplot(df.acs.SL.scat1.lims, mapping=aes(x=l2fc.BM, y=l2fc.acs, color=Category))+
  geom_point(size=.8)+
  scale_color_manual(values=c("Other"="grey70","PPS"="#efb806", "PlastidEncodedPS"="red3", "PHANGs"="#ef06db", "PlastidEncodedRibo+Other"="blue4", "NuclearEncodedPlastidRibo"="dodgerblue1", "CytosolicRibo"="#019d01", "Pt.rpo"="#653f01"), labels=c("Other"="Other", "PPS"="Pentose Phosphate Shunt", "PlastidEncodedPS"="Plastid-encoded photosynthesis-associated", "PHANGs"="Nuclear-encoded photosynthesis-associated", "PlastidEncodedRibo+Other"="Plastid-encoded ribosome +other", "NuclearEncodedPlastidRibo"="Nuclear-encoded plastid ribosome", "CytosolicRibo"="Cytosolic ribosome","Pt.rpo"="Plastid-encoded RNA polymerase"), name="Protein Category")+
  theme_bw()+
  xlab("log2(BxM/midparent)")+
  ylab("log2(acs/B73)")+
...
#df.acs.SL.scat1b[(df.acs.SL.scat1b$Gene %in% PPS),"Category"] <- "PPS"
#df.acs.SL.scat1b[(df.acs.SL.scat1b$Gene %in% PPS),"Order"] <- 2
...
#df.acs.SL.scat1c[(df.acs.SL.scat1c$Gene %in% PPS),"Category"] <- "PPS"
#df.acs.SL.scat1c[(df.acs.SL.scat1c$Gene %in% PPS),"Order"] <- 2
...
#df.acs.SL.scat1d[(df.acs.SL.scat1d$Gene %in% PPS),"Category"] <- "PPS"
#df.acs.SL.scat1d[(df.acs.SL.scat1d$Gene %in% PPS),"Order"] <- 2
...
#df.acs.SL.scat1e[(df.acs.SL.scat1e$Gene %in% PPS),"Category"] <- "PPS"
#df.acs.SL.scat1e[(df.acs.SL.scat1e$Gene %in% PPS),"Order"] <- 2
...
#df.acs.SL.scat1f[(df.acs.SL.scat1f$Gene %in% PPS),"Category"] <- "PPS"
#df.acs.SL.scat1f[(df.acs.SL.scat1f$Gene %in% PPS),"Order"] <- 2
...
df.acs.SL.scat1g[(df.acs.SL.scat1g$Gene %in% PPS),"Category"] <- "PPS"
df.acs.SL.scat1g[(df.acs.SL.scat1g$Gene %in% PPS),"Order"] <- 2
...
fig4g <- ggplot(df.acs.SL.scat1g.lims, mapping=aes(x=l2fc.acs, y=l2fc.BM, color=Category))+
  geom_point(size=.8)+
  scale_color_manual(values=c("Other"="grey70","PPS"="#efb806", "PlastidEncodedPS"="red3", "PHANGs"="#ef06db", "PlastidEncodedRibo+Other"="blue4", "NuclearEncodedPlastidRibo"="dodgerblue1", "Pt.rpo"="#653f01"), labels=c("Other"="Other", "PPS"="Pentose Phosphate Shunt", "PlastidEncodedPS"="Plastid-encoded photosynthesis-associated", "PHANGs"="Nuclear-encoded photosynthesis-associated", "PlastidEncodedRibo+Other"="Plastid-encoded ribosome +other", "NuclearEncodedPlastidRibo"="Nuclear-encoded plastid ribosome", "Pt.rpo"="Plastid-encoded RNA polymerase"), name="Protein Category")+
  theme_bw()+
  xlab("log2(acs/B73)")+
  ylab("log2(BxM/MP)")+
  geom_abline(slope = 1, color="grey50", size=.5)+
...
#add marginal density plots and resave
    densityplotX<-axis_canvas(fig4g, axis="x")+
      ...
      scale_color_manual(values=c("Other"="grey70","PPS"="#efb806", "PlastidEncodedPS"="red3", "PHANGs"="#ef06db", "PlastidEncodedRibo+Other"="blue4", "NuclearEncodedPlastidRibo"="dodgerblue1", "Pt.rpo"="#653f01"))+
      geom_vline(xintercept=0, linetype="dashed", color = "gray50")+
      theme_void()+
      theme(legend.title = element_blank())+
      ylim(0,15)
